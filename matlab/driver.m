...
%% APROACH 2
%  This method of driving doesn't work as well for some reason, use driver
%  2 to run.
% function performApproach2Autotune_OneChannel(audiofilename, audiofileext, targetPitches)
%     [ogSignal, Fso] = audioread(['../sounds/', audiofilename, '.', audiofileext]);
%     obj = AutoTuner(ogSignal, targetPitches(1), Fso, round(Fso / 50));
%     obj.postSamplingTuning_SingleChannel_Approach2(audiofilename, targetPitches);
%     obj.soundOut();
% end
% 
% function performApproach2Autotune_TwoChannel(audiofilename, audiofileext, targetPitches)
%     [ogSignal, Fso] = audioread(['../sounds/', audiofilename, '.', audiofileext]);
%     obj = AutoTuner(ogSignal, targetPitches(1), Fso, round(Fso / 50));
%     obj.postSamplingTuning_TwoChannel_Approach2(audiofilename, targetPitches);
%     obj.soundOut();
% end