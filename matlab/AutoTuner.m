classdef AutoTuner
    ...
    methods (Access = public)
        ...
        function obj = postSamplingTuning_SingleChannel_Approach2(obj, name, targetPitches)
            [ts Fs] = tuneSampled(obj.originalSig, obj.sampledAtFreq, obj.windowSize, 1024, name, targetPitches);
            obj.tuned = ts;
        end
        
        function soundOut(obj)
            sound(obj.tuned, obj.sampledAtFreq);
        end
        
        function obj = postSamplingTuning_TwoChannel_Approach2(obj, name, targetPitches)
            obj.originalSig = obj.originalSig(:,1) + obj.originalSig(:,2);
            [ts Fs] = tuneSampled(obj.originalSig, obj.sampledAtFreq, obj.windowSize, 1024, name, targetPitches);
            obj.tuned = ts;
        end
